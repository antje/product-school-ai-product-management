...
The other two frames are not used. **Revenue generation** would put an inline assist at the moment of decision, but Juno's decision moment belongs to the PM and inserting a button into it adds a step. **User delight** would mean anticipatory dismissible suggestions, which is the pattern most likely to train someone to ignore the product. A P0 triage that is easy to dismiss is a P0 triage nobody reads.

## Trust gaps

Scored on how far each gap is closed, not on how serious it would be if open.

**5** closed, no known residual. **4** closed on the common path, with a named residual that is accepted. **3** partly closed: the user can act, but cannot fully steer. **2** mitigated by convention only, nothing enforces it. **1** open.


| Gap | Score | Where it shows up | User cost | Mitigation |
|---|---|---|---|---|
| Black-box, no visible "why" | 4/5 | The shortlist card, where an item carries a priority badge and a rank | The PM cannot defend the order in the thread, which is the job | Every item quotes the strategy clause behind its priority and shows the evidence line, "4 accounts, 2 enterprise". No item is drafted without a clause to quote |
...
| Intelligence tax | 5/5 | The 45 seconds between the trigger firing and the shortlist arriving. Retrieval is budgeted at 4s p95 per call, the whole run at 45s wall clock | Normally, waiting. Here, close to nothing | The wait is asynchronous: the PM is not watching a screen, the shortlist arrives in a thread they return to. One status reply is posted at the trigger and edited in place as each step finishes, then becomes the shortlist. No token streaming and no second message, because a bot that posts repeatedly turns the thread it is helping into one nobody can read |

### What is still open on each

**Black-box, 4/5.** The PM sees the clause that justified an item, but not what Juno retrieved and discarded. An item ranked third because a competing item had more affected accounts shows its own count, not the comparison. Closing this means surfacing the runners-up, which costs card space, so it stays open for now.

**Hallucination, 4/5.** A verbatim clause match proves the clause exists and is quoted accurately. It does not prove the clause supports the rank it carries. That check is the LLM-judge layer, which samples 20 items nightly rather than gating every run, so a clause that is real but irrelevant can reach the PM between grading rounds.

**Control, 3/5.** The PM can stop any single item and any single run. What they cannot do is tell Juno not to raise something again. Semantic memory holds no learned view of which accounts or items matter, because that is the memory that turns into bias over a few weeks. The cost of that decision lands here: a rejected item can return next week at the same rank, with the same evidence, because the evidence has not changed.
