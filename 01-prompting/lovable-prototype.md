...
Built with the four part anatomy from Module 1. Role, Task, Format and Constraints, written in the lab tool and pasted into Lovable in one go.
...
```
Refine the dashboard UI to match the aesthetic of Linear. Use their specific colour
...
- **What worked:** The prompt did exactly what I asked. Three equal columns that do not reflow, the Process button always visible, no nav bar, no login screen, dark mode with one accent colour. The output looks like real PM work. Every card has a priority tag, a sentiment tag and the quote it came from. One follow up prompt was enough to move the whole thing to a Linear style palette without breaking the layout.
